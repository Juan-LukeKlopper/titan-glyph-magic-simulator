// snow.glsl - Glyph Magic Simulator shader burst (ice spell).
//
// Adapted from the user-provided layered snowflake raymarch by
// Panteleymonov Aleksandr Konstantinovich (2015). Changes for this project:
//   * uniform preamble + Shadertoy-style main() wrapper appended (GLSL ES
//     1.00; WebGL1 loop/constant bounds unchanged - the #define iteration
//     counts are all compile-time constants).
//   * the flake field is re-centred on the cast point (u_castPos); the
//     mouse branch stays (iMouse.z is 0 here) so the reference's gentle
//     auto-drift plays.
//
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Re-centre the flake field on the cast point instead of the screen
    // middle; u_castPos == (0.5, 0.5) reproduces the reference exactly.
    ...
    // Snow pieces only: drop the reference's sky-blue wash and opaque
    // background. Alpha is driven by the accumulated flake brightness so the
    // existing canvas shows through the gaps between crystals.
    vec3 flakeCol = min(fragColor.rgb * 2.4, vec3(1.0));
    float lum = max(flakeCol.r, max(flakeCol.g, flakeCol.b));
    float coverage = clamp(lum * 1.7, 0.0, 1.0);
    fragColor = vec4(flakeCol, coverage);
}
